...
//AIRFOIL DIMENSIONS
CHORDS[] = {BASEDIM/3,BASEDIM/3,BASEDIM/3};
GLOBALAOA = 30*dtr;
ANGLES[] = {-15*dtr,0*dtr,15*dtr};//angles of attack for each element.
LEDISTANCES[] = {0.1,0.1,0.1};//distance from spline point to leading edge, as a factor of the chord.
TEDISTANCES[] = {0.1,0.1,0.1};
EDGEANGLE = 15*dtr;
LEANGLES[] = {EDGEANGLE,EDGEANGLE,EDGEANGLE};
TEANGLES[] = {EDGEANGLE,EDGEANGLE,EDGEANGLE};
LEX[] = {-0.25*BASEDIM,1/12*BASEDIM,5/12*BASEDIM};
LEY[] = {0,0,-0.09*BASEDIM};
...
ARCNORMAL = {};
For i In {0:#CHORDS[]-1}
	ARCNORMAL[i] = CHORDS[i]/2.0/Tan(LEANGLES[i]);
EndFor
...
For i In {0:NE-1}
	...
	Point(ce++)={LEX[i]+CHORDS[i]/2.0,LEY[i]-ARCNORMAL[i],0,SURFACECL};arccenter=ce;
	//le spline.
	...
	//local aoa rotation
	points[] = {arccenter,splinepoints[],leblpoints[],teblpoints[],topblsplinepoints[],bottomblsplinepoints[]};
	rc[] = {LEX[i]+CHORDS[i]*0.25,LEY[i],0};angle=-ANGLES[i];
	Call rotatePoints2D;
	//global aoa rotation
	rc[] = {0,0,0};angle=-GLOBALAOA;
	Call rotatePoints2D;
	//BSpline(ce++)=splinepoints[];splineline=ce;
	Circle(ce++)={splinepoints[0],arccenter,splinepoints[3]};splineline=ce;
	...
	Circle(ce++)={leblpoints[0],arccenter,teblpoints[0]};blsplinelines[]+=ce;
	Circle(ce++)={leblpoints[4],arccenter,teblpoints[4]};blsplinelines[]+=ce;
