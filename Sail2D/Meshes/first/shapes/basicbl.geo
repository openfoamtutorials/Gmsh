...
DFA = 00*Pi/180;//rad, duct flare angle.
BLL = DC/20/3.5;//m, boundary layer length.
BLC = 7;//m, boundary layer cells.
BLP = 1.2;//boundary layer progression.
TEMAD = -0*Pi/180;//m, te mesh angle addition.
...
Point(ce++)={
	innerx-LR*2+(1-Cos(OPAA))*LR+BBOL*Sin(OPAA),
	start-Sin(OPAA)*LR-BBOL*Cos(OPAA),
	0,DLS};oslp=ce;
...
BSpline(ce++)={bp,oslp,op};ductlines[]+=ce;
...
Point(ce++)={
	innerx-LR*2-BLL+(1-Cos(OPAA))*(LR+BLL)+BBOL*Sin(OPAA),
	start-Sin(OPAA)*(LR+BLL)-BBOL*Cos(OPAA),
	0,DLS};oslp2=ce;
//foo[]=Point{cp};
...
EndFor
kk=#pts[]-4;
BSpline(ce++)={pts[kk],oslp2,pts[(kk+1)%(#pts[])]};tangentlines[]+=ce;
